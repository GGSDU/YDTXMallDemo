...
- (instancetype)initWithFrame:(CGRect)frame orderModel:(OrderGoodsModel *)model {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        }else if ([style isEqualToString:@"待付款"]){
            
            self.statusImgV.image = [UIImage imageNamed:@"WaitStatusImg"];
            self.statusLabel.text = style;
            self.descriptionLabel.text = @"再不付款就成别人的啦~";
            
        } else if ([style isEqualToString:@"待收货"]) {
            
            self.statusImgV.image = [UIImage imageNamed:@"waitReceiving"];
            self.statusLabel.text = style;
            self.descriptionLabel.text = @"您的宝贝已在路上~";
            
        } else if ([style isEqualToString:@"退款中"]) {
            
            self.statusImgV.image = [UIImage imageNamed:@"drawBacking"];
            self.statusLabel.text = style;
            self.descriptionLabel.text = @"正在努力的退款中~";
            
        } else if ([style isEqualToString:@"退款"]) {
            
            self.statusImgV.image = [UIImage imageNamed:@"drawBackSucceed"];
            self.statusLabel.text = style;
            self.descriptionLabel.text = @"再逛逛吧~";
        }
