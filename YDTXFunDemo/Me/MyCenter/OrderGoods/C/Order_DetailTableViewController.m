...
@interface Order_DetailTableViewController ()<OrderPayStatusDelegate>
...
- (void) loadNewData {
    ...
    [self.AFManager GET:[postHttp stringByAppendingString:@"api/goodsOrder/orderDetail/"] parameters:param progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        NSLog(@"order detail message is :%@--------%@",responseObject,param);
...
- (void) orderBtnAction:(UIButton *)sender {
    ...
    params[@"userid"] = @"72";
    params[@"goods_order_id"] =  self.goods_order_id;
    ...
    switch (sender.tag) {
        case 10://去支付
        {
            ...
            if (self.dataSource.count > 0) {
                ...
                payOnline.orderParamsDic = @{@"tradeNum":model.goods_order_id,@"subject":model.goods_name,@"sumPrice":model.total_price};
        ...
        case 11://取消订单
        {
            ...
            [orderPayStatus orderPayCancel:params url:@"api/goodsOrder/orderCancel"];
        ...
        case 12:// 删除订单
        {
            ...
            //            self.goods_order_id
            //            //删除订单
            //        http://test.m.yundiaoke.cn/api/goodsOrder/ordermyDel/userid/72/goods_order_id/9
            //            请求方式：GET
            //            参数：userid：用户id，goods_order_id：订单id
            
            OrderPayStatus *orderStatus = [[OrderPayStatus alloc]init];
            [orderStatus orderPayDelect:params url:@"api/goodsOrder/ordermyDel/"];
            NSLog(@"删除订单");
        }
            break;
