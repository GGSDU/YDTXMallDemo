...
- (void)getDetailData:(LogisticsDetailModel *)detailModel isFirst:(BOOL)isFirst isLast:(BOOL)isLast {
    
    self.lineView1.hidden = NO;
    self.lineView2.hidden = NO;
    self.circleImgV.image = [UIImage imageNamed:@"物流消息"];
    self.addressLabel.textColor = RGB(141, 141, 141);
    self.creat_timeLabel.textColor = RGB(141, 141, 141);
    
    self.addressLabel.text = detailModel.context;
    self.creat_timeLabel.text = detailModel.time;
    
    
    if (isFirst) {
        self.addressLabel.textColor = RGB(65, 201, 239);
        self.creat_timeLabel.textColor = RGB(65, 201, 239);
//        物流最新
        self.circleImgV.image = [UIImage imageNamed:@"物流最新"];
