...
- (void) btnAction:(UIButton *) sender {
    //    userid 72
    //    NSLog(@"---%@----%@----%@---%@",self.goods_name,self.order_id,self.goods_id,self.money);
    
    NSMutableDictionary *param = [NSMutableDictionary dictionary];
    param[@"userid"] = @"72";
    param[@"order_id"] = self.order_id;
    param[@"goosd_id"] = self.goods_id;
    param[@"goods_name"] = self.goods_name;
    param[@"price"] = self.price;
    param[@"total_price"] = self.total_price;
    
//    self.goods_model_id
//    self.nums
    param[@"goosd_model_id"] = self.goods_model_id;
    param[@"nums"] = self.nums;
    ...
    NSString *str = nil;
    if (self.statusMessage.text.length > 0) {
        if ([self.statusMessage.text isEqualToString:@"已收货"]) {
            str = @"1";
        }else {
            str =@"0";
        }
        param[@"is_recipient"] = str;
        
        
        if (self.imageArr.count>0 && self.textView.text.length>0) {
            param[@"images"] = self.imageArr;
            param[@"content"] = self.textView.text;
            
            //////////////////////
            [self.AFManager POST:[postHttp stringByAppendingString:@"api/goods/refund"] parameters:param progress:^(NSProgress * _Nonnull uploadProgress) {
                ...
                if ([responseObject[@"status"] isEqual:@200]) {
                    ...
                    [self.navigationController pushViewController:[[OrderSubmitViewController alloc]init] animated:YES];
                }
    ...
    ////////////////////////
        ...
        [SVProgressHUD showErrorWithStatus:@"请填写完整内容"];
    }
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == self.index) {
        cell.imgView.image = [UIImage imageNamed:@"发帖加号图"];
