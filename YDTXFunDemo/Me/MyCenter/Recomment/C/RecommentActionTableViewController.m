//
//  RecommentActionTableViewController.m
//  YDTX
//
...
@interface RecommentActionTableViewController ()
...
@implementation RecommentActionTableViewController
    ...
    [self.AFManager GET:[postHttp stringByAppendingString:@"api/partner/myRecommend/"] parameters:param progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if ([responseObject[@"status"] isEqual:@200]) {
            ...
            for (NSDictionary *dic in responseObject[@"data"]) {
                RecommentActionModel *model = [[RecommentActionModel alloc]initData:dic];
        ...
        NSLog(@"recommentAction is:%@",responseObject);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    RecommentActionTableViewCell *cell = [tableView  dequeueReusableCellWithIdentifier:cellID];
    
    if (!cell) {
        cell = [[RecommentActionTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID];
    }
    RecommentActionModel *model = self.dataSource[indexPath.row];
